...
func _on_army_movement_mode_requested(piece_ids: Array[StringName]) -> void:
	print("Requested army movement mode.", piece_ids)
...
func _switch_to_army_movement_mode() -> void:
	_current_mode.queue_free()
	var mode_scene: Node = army_movement_scene.instantiate()
	# (mode_scene as ArmyMovementMode).configure()
